      ...
      <div class="d-flex align-items-center">
        ...
        <div class="border border-dark mx-auto my-3 px-2 py-1 rounded-pill d-none d-lg-flex">
          <div class="d-flex align-items-center position-relative">
            ...
            <input type="text" wire:model.live.debounce.300ms="searchTerm" class="form-control form-control-sm rounded-pill border-0 pe-4" style="box-shadow: none !important; min-width: 400px;" placeholder="Cari berdasarkan Kode Supplier atau Nama Supplier">
    ...
    <div class="card-body">
      ...
      <div class="rounded p-3" style="border: 1px solid #8888888e;">
        ...
        <div class="d-lg-flex justify-content-between align-items-center mb-3">
          {{-- Filter --}}
          <div class="d-flex align-items-center me-lg-3 mb-3 mb-lg-0 position-relative">
            @if (Auth::user()->hasRole('superadmin'))

            {{-- <div class="bg-white position-absolute px-1" style="left: 6.45em; margin-top: .4em; z-index: 9;">
              <div style="width: 200px !important; overflow-x: hidden; white-space: nowrap;">
                <div class="font-poppins text-grey">
                  {{ $selected_filter }}
                </div>
              </div>
            </div> --}}
          ...
          <div class="d-lg-flex align-items-center">
            ...
            <div class="d-flex align-items-center">
              ...
              <select wire:model.live.debounce.300ms="sorting_data" class="form-control form-control-sm form-select border border-dark" style="width: 100%;">
                ...
                <option value="nama_supplier"> Berdasarkan Nama </option>
              </select>
            </div>
            {{-- End --}}
          </div>
          ...
          <table class="table table-hover text-center  table-striped font-poppin position-relative border">
            <thead class="bg-secondary text-white font-weight-bold sticky-top">
              <tr>
                ...
                <td style="min-width: 150px;">Kode Supplier</td>
                <td style="min-width: 250px;">Nama Supplier</td>
                <td>Telepon</td>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>[{{ $supplier->unit_kerja->kode_unit }}] {{ $supplier->unit_kerja->nama_unit }}</td>
                ...
                <td style="min-width: 300px;">
                  <div class="d-flex justify-content-center">
                    <div class="border border-secondary rounded-pill p-1 w-fit d-flex">
                      <button class="btn btn-secondary rounded-pill btn-sm" data-bs-toggle="modal" data-bs-target="#editData" wire:click="edit({{ $supplier->id }})">
                        <div class="d-flex align-items-center">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" style="height: 20px; width: 20px;">
                      ...
                      {{-- Hapus Data --}}
                      <button class="btn btn-red-2 rounded-pill btn-sm ms-3" data-bs-toggle="modal" data-bs-target="#deleteData" wire:click="edit({{ $supplier->id }})">
                        <div class="d-flex align-items-center">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" style="height: 20px; width: 20px;">
        ...
        <div class="paginateContainer d-flex flex-column flex-md-row justify-content-center justify-content-md-between align-items-center mt-4 mb-2 mx-2">
          <span style="margin-top: -1.5em">
            Showing {{ $suppliers->firstItem() }} to {{ $suppliers->lastItem() }} of total {{$suppliers->total()}} entries
          </span>
          <span>
            {!! $suppliers->links() !!}
          </span>
    ...
    <div class="modal-dialog modal-dialog-scrollable modal-md">
      <div class="modal-content">
        <div class="modal-header bg-main text-white">
          <h1 class="modal-title fs-5" id="exampleModalToggleLabel">Tambah Supplier</h1>
        ...
        <div class="modal-body">
          ...
          <div class="position-relative mb-3">
            <div class="bg-white position-absolute pe-3" style="left: .65em; margin-top: 2.1em; z-index: 99;">
              <div class="font-poppins text-grey">{{$selected_unit}}</div>
            </div>
            <div wire:ignore class="form-group ">
              <div>Unit Kerja</div>
              <select id="select2-dropdown-add" style="width: 100%;">
                <option value="">---</option>
                @foreach ($office_units as $unit)
          ...
          <div class="form-group mb-3">
            ...
            <input type="number" wire:model="kode_supplier" class="form-control border border-dark" pattern="/^-?\d+\.?\d*$/" onKeyPress="if(this.value.length==4) return false;" />
            @error('kode_supplier') <small class="text-danger">{{$message}}</small> @enderror
          </div>
          <div class="form-group mb-3">
            <div>Nama Supplier</div>
            <input type="text" wire:model="nama_supplier" class="form-control border border-dark">
            @error('nama_supplier') <small class="text-danger">{{$message}}</small> @enderror
          </div>
          <div class="form-group mb-3">
            <div>Telepon</div>
            <div class="d-flex align-items-center">
              <div class="border border-dark rounded me-2" style="padding: .35em .5em;">
                +{{ $country_code }}
              </div>
              <input type="number" wire:model="telepon" class="form-control border border-dark" pattern="/^-?\d+\.?\d*$/" onKeyPress="if(this.value.length==11) return false;">
            </div>
          ...
          <div class="table-responsive table-responsive-sm table-responsive-md table-responsive-lg table-responsive-xl secondContainer">
            <table class="table table-hover text-center table-striped font-poppins border">
              <thead class="bg-secondary text-white font-weight-bold sticky-top">
                <tr>
                  <th>No.</th>
                  <th style="min-width: 100px;">Unit Kerja</th>
                  <th style="min-width: 150px;">Kode Supplier</th>
                  <th style="min-width: 250px;">Nama Supplier</th>
                  <th>Telepon</th>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td class="pt-3" style="min-width: 250px;"> [{{ $arc_data->unit_kerja->kode_unit }}] {{ $arc_data->unit_kerja->nama_unit }} </td>
                  <td class="pt-3">
                    {{ $arc_data->kode_supplier }}
                  </td>
                  <td class="column-scrolled pt-3" style="min-width: 150px; max-width: 250px;">
                    {{ $arc_data->nama_supplier }}
                  </td>
                  <td class="pt-3">
                    {{ $arc_data->telepon }}
                  </td>
...
<script>
  ...
  $(document).ready(function() {
    $('#select2-dropdown-add').select2({
      dropdownParent: $("#tambahData")
    });

    $('#select2-dropdown-add').on('change', function(e) {
      var data = $('#select2-dropdown-add').select2('val');

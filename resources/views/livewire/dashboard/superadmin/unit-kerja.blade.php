@section('unit-kerja-active') active @endsection
...
<nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb" class="mt-3 ms-3">
  <ol class="breadcrumb font-poppins">
    ...
    <li class="breadcrumb-item text-main" aria-current="page">Daftar Unit Kerja</li>
  </ol>
</nav>
@endsection
...
<div>
  ...
  <div class="card position-relative" style="border-left: 1px solid #888; border-right: 1px solid #888; border-bottom: 1px solid #888;">
    ...
    <div class="card-body">
      ...
      <div class="d-flex align-items-center text-start ps-3 font-poppins px-3 py-2 w-fit mb-3 rounded" style="border: 1px solid #8888888e;">
        ...
        @if ($archive_count > 0 )
        <span>
          <button class="btn btn-sm btn-outline-blue rounded-pill ms-3" data-bs-toggle="modal" data-bs-target="#archievedData">
            Lihat Data
          </button>
        </span>
        @endif
      ...
      <div class="rounded p-3" style="border: 1px solid #8888888e;">
          ...
          <div class="d-lg-flex align-items-center ms-auto me-3">
            ...
            <div class="d-flex align-items-center">
              ...
              <select wire:model.live.debounce.300ms="sorting_data" class="form-control form-control-sm form-select border border-dark" style="width: 100%;">
                ...
                <option value="nama_unit"> Berdasarkan Nama </option>
              </select>
            </div>
            {{-- End --}}
          </div>
        ...
        <div class="table-responsive table-responsive-sm table-responsive-md table-responsive-lg table-responsive-xl secondContainer">
          <table class="table table-hover text-center table-striped font-poppins border">
            <thead class="bg-secondary text-white font-weight-bold sticky-top">
              <tr>
                <th>No.</th>
                <th style="min-width: 200px;">Kode Unit Kerja</th>
                <th style="min-width: 200px;">Nama Unit Kerja</th>
                <th> # </th>
              </tr>
            </thead>
            ...
            <tbody>
              ...
              <tr>
                <td class="pt-3">{{ $office_units->firstItem() + $loop->index }}.</td>
                <td class="pt-3" style="min-width: 150px;"> {{ $unit->kode_unit }} </td>
                <td class=" pt-3" style="min-width: 150px; max-width: 250px;">
                  {{ $unit->nama_unit }}
                </td>
                <td>
                  <div class="d-flex justify-content-center">
                    <div class="border border-secondary rounded-pill p-1 w-fit d-flex">
                      <button class="btn btn-secondary rounded-pill btn-sm" data-bs-toggle="modal" data-bs-target="#editData" wire:click="edit({{ $unit->id }})">
                        <div class="d-flex align-items-center">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" style="height: 20px; width: 20px;">
                      ...
                      {{-- Hapus Data --}}
                      <button class="btn btn-red-2 rounded-pill btn-sm ms-3" data-bs-toggle="modal" data-bs-target="#deleteData" wire:click="edit({{ $unit->id }})">
                        <div class="d-flex align-items-center">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" style="height: 20px; width: 20px;">
        ...
        <div class="paginateContainer d-flex flex-column flex-md-row justify-content-center justify-content-md-between align-items-center mt-4 mb-2 mx-2">
          <span style="margin-top: -1.5em">
            Showing {{ $office_units->firstItem() }} to {{ $office_units->lastItem() }} of total {{$office_units->total()}} entries
          </span>
          <span>
            {!! $office_units->links() !!}
          </span>
  ...
  <!-- Modal Add -->
  <div wire:loading.class="d-none" wire:ignore.self class="modal fade" id="tambahData" aria-hidden="true" aria-labelledby="exampleModalToggleLabel" tabindex="-1">
    <div class="modal-dialog modal-dialog-scrollable modal-md">
      <div class="modal-content">
        <div class="modal-header bg-main text-white">
          <h1 class="modal-title fs-5" id="exampleModalToggleLabel">Tambah Unit Kerja</h1>
        ...
        <div class="modal-body">
          <div class="form-group mb-3">
            <div>Kode Unit Kerja</div>
            <input type="number" wire:model="kode_unit" class="form-control border border-dark" pattern="/^-?\d+\.?\d*$/" onKeyPress="if(this.value.length==3) return false;" />
            @error('kode_unit') <small class="text-danger">{{$message}}</small> @enderror
          </div>
          <div class="form-group mb-3">
            <div>Nama Unit Kerja</div>
            <input type="text" wire:model="nama_unit" class="form-control border border-dark">
            @error('nama_unit') <small class="text-danger">{{$message}}</small> @enderror
          </div>
        </div>
  ...
  <!-- Modal Edit -->
  <div wire:loading.class="d-none" wire:ignore.self class="modal fade" id="editData" aria-hidden="true" aria-labelledby="exampleModalToggleLabel" tabindex="-1">
    <div class="modal-dialog modal-dialog-scrollable modal-md">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          ...
          <input type="submit" class="btn btn-purple-2" value="Update Data" wire:click="update()">
        </div>
      </div>
    </div>
  </div>
            ...
            <table class="table table-hover text-center table-striped font-poppins border">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td class="pt-3" style="min-width: 150px;"> {{ $arc_data->kode_unit }} </td>
